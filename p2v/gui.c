...
static GtkWidget *conv_dlg,
  ...
  *o_combo, *oc_entry, *os_entry, *of_combo, *oa_combo,
  *info_label,
...
create_connection_dialog (struct config *config)
{
  ...
  gtk_window_set_title (GTK_WINDOW (conn_dlg), "书生云物理机迁移工具");
  gtk_window_set_resizable (GTK_WINDOW (conn_dlg), FALSE);

  /* The main dialog area. */
  intro = gtk_label_new (_("通过SSH连接迁移服务器:"));
  gtk_label_set_line_wrap (GTK_LABEL (intro), TRUE);
  set_padding (intro, 10, 10);

  table_new (table, 5, 2);
  server_label = gtk_label_new_with_mnemonic (_("书生云迁移服务器:"));
  ...
  username_label = gtk_label_new_with_mnemonic ("用户名:");
  ...
  password_label = gtk_label_new_with_mnemonic ("密码:");
  ...
  sudo_button =
    gtk_check_button_new_with_mnemonic ("使用sudo");
  ...
  test = gtk_button_new_with_mnemonic ("连接");
...
about_button_clicked (GtkWidget *w, gpointer data)
{
  ...
  g_object_set (G_OBJECT (dialog),
                "program-name", "书生云物理机迁移工具",
                "version", "1.0",
                "copyright", "\u00A9 2009-2017 北京书生云科技有限公司",
                "comments",
                  _("物理机迁移到书生云平台"),
...
create_conversion_dialog (struct config *config)
{
  ...
  gtk_window_set_title (GTK_WINDOW (conv_dlg), "书生云物理机迁移工具");
  ...
  vcpus_label = gtk_label_new_with_mnemonic (_("CPU个数:"));
  ...
  memory_label = gtk_label_new_with_mnemonic (_("内存(MB):"));
  ...
  gtk_widget_set_tooltip_markup (o_combo, _("<b>本地</b> 迁移到迁移服务器上KVM可以管理的虚拟机。  <b>云平台</b> 通过迁移服务器直接放到缺省配置的openstack云平台。"));
  repopulate_output_combo (config);
  table_attach (output_tbl, o_combo,
                1, 2, 0, 1, GTK_FILL, GTK_FILL, 1, 1);

#if 1  //william edited this code block!
//oc options 
...
//of options
/*
  of_label = gtk_label_new_with_mnemonic (_("输出格式:"));
  ...
  gtk_widget_set_tooltip_markup (of_entry, _("磁盘的输出格式, 典型的有 <b>raw</b> or <b>qcow2</b>. 如果为空则默认为 <b>raw</b>."));
...
//
  ...
  disks_frame = gtk_frame_new (_("硬盘"));
  ...
  interfaces_frame = gtk_frame_new (_("网卡"));
  ...
  gtk_widget_set_tooltip_markup (interfaces_list, _("左击选择网卡以激活。"));
...
repopulate_output_combo (struct config *config)
{
  ...
  /* List of -o options read from remote virt-v2v --machine-readable. */
    ...
    for (i = 0; output_drivers[i] != NULL; ++i)
      if (STREQ ("libvirt", output_drivers[i])) {
		gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (o_combo),
                                      "本地");
		indexOfLocal = i;
      }else if (STREQ ("glance", output_drivers[i])) {
		gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (o_combo),
                                      "云平台");
		indexOfGlance = i;
      }

      if(indexOfGlance>indexOfLocal) indexOfLocal = 0;
      else indexOfLocal = 1;
    if (output) {
      if (STREQ (output, "本地"))
          gtk_combo_box_set_active (GTK_COMBO_BOX (o_combo), indexOfLocal);
      else if (STREQ (output, "云平台"))
	  gtk_combo_box_set_active (GTK_COMBO_BOX (o_combo), 1-indexOfLocal);
      else 
          gtk_combo_box_set_active (GTK_COMBO_BOX (o_combo), indexOfLocal);
    }
    else
...
create_running_dialog (void)
{
  ...
  gtk_window_set_title (GTK_WINDOW (run_dlg), "书生云物理机迁移工具");
 ...
 */
static gboolean
add_v2v_output1 (gpointer user_data)
{
  CLEANUP_FREE const char *msg = user_data;
  const char *p;
...
start_conversion_clicked (GtkWidget *w, gpointer data)
{
  ...
  if (STREQ (config->guestname, "")) {
    dlg = gtk_message_dialog_new (GTK_WINDOW (conv_dlg),
                                  ...
                                  GTK_BUTTONS_OK,
                                  _("虚拟机名字为空。"));
    gtk_window_set_title (GTK_WINDOW (dlg), _("错误"));
  ...
  /* Output selection. */
  free (config->output);
  //config->output = "libvirt"; 
  config->output =
    gtk_combo_box_text_get_active_text (GTK_COMBO_BOX_TEXT (o_combo));

  if (STREQ ("本地", config->output)) config->output = "libvirt";
  else if (STREQ ("云平台", config->output)) config->output = "glance";
  else config->output = "libvirt";
...
conversion_error (gpointer user_data)
{
  ...
  dlg = gtk_message_dialog_new (GTK_WINDOW (run_dlg),
                                ...
                                GTK_BUTTONS_OK,
                                _("迁移失败: %s"), err);
  gtk_window_set_title (GTK_WINDOW (dlg), _("迁移失败"));
  gtk_dialog_run (GTK_DIALOG (dlg));
  gtk_widget_destroy (dlg);
...
conversion_finished (gpointer user_data)
{
  ...
  dlg = gtk_message_dialog_new (GTK_WINDOW (run_dlg),
                                ...
                                GTK_BUTTONS_OK,
                                _("迁移成功."));
  gtk_window_set_title (GTK_WINDOW (dlg), _("迁移成功"));
  gtk_dialog_run (GTK_DIALOG (dlg));
  gtk_widget_destroy (dlg);
...
const char *OPEN_SOURCE = "Opening the source";
const char *SOURCE_PREPARATION_START = "Creating an overlay";
const char *SOURCE_PREPARATION_END = "Closing the overlay";
const char *DISK_START = "Assigning disks";
const char *CONVERSION_PROGRESS = "100%";
const char *CONVERSION_COMPLETE = "Finishing off";
...
fprintf (stdout,
  ...
  case NOTIFY_REMOTE_MESSAGE:
    if(strstr(copy,OPEN_SOURCE)) {
        temp = strdup("[1] 打开迁移源\n");
    	g_idle_add (add_v2v_output, (gpointer) temp);
    } else if(strstr(copy,SOURCE_PREPARATION_START)) {
    	temp = strdup("[2] 迁移源保护准备\n");
    	g_idle_add (add_v2v_output, (gpointer) temp);
    }else if(strstr(copy,SOURCE_PREPARATION_END)) {
	temp = strdup("[3] 迁移源保护结束\n");
    	g_idle_add (add_v2v_output, (gpointer) temp);
    }else if(strstr(copy,DISK_START)) {
	temp = strdup("[4] 拷贝磁盘开始\n");
    	g_idle_add (add_v2v_output, (gpointer) temp);
	temp = strdup(" \n");
    	g_idle_add (add_v2v_output, (gpointer) temp);
    }else if(strstr(copy,CONVERSION_PROGRESS)) {
    	g_idle_add (add_v2v_output, (gpointer) copy);
    }else if(strstr(copy,CONVERSION_COMPLETE)) {
	temp = strdup("[5] 迁移结束\n");
    	g_idle_add (add_v2v_output, (gpointer) temp);
    }/*else 
	g_idle_add (add_v2v_output, (gpointer) copy);*/
...
cancel_conversion_dialog (GtkWidget *w, gpointer data)
{
  ...
  dlg = gtk_message_dialog_new (GTK_WINDOW (run_dlg),
                                ...
                                _("真的想要取消迁移? "
                                  "要迁移的话"
                                  "需要重新开始."));
  gtk_window_set_title (GTK_WINDOW (dlg), _("取消迁移"));
