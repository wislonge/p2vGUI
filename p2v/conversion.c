                  ...
                  void (*notify_ui) (int type, const char *data))
{
  ...
  for (i = 0; config->disks[i] != NULL; ++i) {
    ...
    if (data_conns[i].nbd_pid == 0) {
      set_conversion_error ("启动磁盘服务错误: %s", get_nbd_error ());
      goto out;
    }
    ...
    if (data_conns[i].h == NULL) {
      ...
      set_conversion_error ("磁盘数据连接失败: %s", err);
      goto out;
    }
  ...
  if (notify_ui)
    notify_ui (NOTIFY_STATUS, _("设置数据连接..."));

  set_control_h (start_remote_connection (config, remote_dir));
  if (control_h == NULL) {
    set_conversion_error ("设置数据连接失败: %s",
                          get_ssh_error ());
    goto out;
  }
  ...
  if (notify_ui)
    notify_ui (NOTIFY_STATUS, _("开始迁移..."));
  ...
  if (is_cancel_requested ()) {
    set_conversion_error ("用户取消");
    if (notify_ui)
      notify_ui (NOTIFY_STATUS, _("用户取消迁移"));
    goto out;
  }

  if (notify_ui)
    notify_ui (NOTIFY_STATUS, _("服务器取消数据连接"));
